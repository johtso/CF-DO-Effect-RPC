...
class RpcHelloClient extends Effect.Service<RpcHelloClient>()(
  "RpcHelloClient",
  {
...
const customFetchHttpClient = (_fetch: typeof globalThis.fetch) => FetchHttpClient.layer.pipe(
  Layer.provide(
    Layer.succeed(FetchHttpClient.Fetch, _fetch)
  )
)
...
export default {
  async fetch(request, env, ctx): Promise<Response> {
    ...
    const RpcHelloClientLive = RpcHelloClient.Default.pipe(
      Layer.provide(RpcProtocol.pipe(
        Layer.provide(customFetchHttpClient(stub.fetch))
      ))
    )
